...
def _read_data_from_s3(**kwargs):
    ...
    bucket_name = "spotify-etl-project-artika"
...
store_raw_to_s3 = S3CreateObjectOperator(
    ...
    s3_bucket="spotify-etl-project-artika",
...
process_artists = PythonOperator(
    task_id="process_artists",
    python_callable=_process_artists,
    provide_context=True,
    dag=dag
)

store_artists_to_s3 = S3CreateObjectOperator(
    task_id="store_artists_to_s3",
    aws_conn_id="aws_s3_bucket",
    s3_bucket="spotify-etl-project-artika",
    s3_key='transformed_data/artists_data/artists_transformed_{{ts_nodash}}.csv',
    data='{{task_instance.xcom_pull(task_ids="process_artists", key="artist_content")}}',
    dag=dag,
)
...
read_data_from_s3 >> process_artists >> store_artists_to_s3
read_data_from_s3 >> process_songs >> store_songs_to_s3
[store_album_to_s3, store_artists_to_s3, store_songs_to_s3] >> move_processed_data
